...
export default function BarcodeScanner() {
  ...
  async function postImageToBackend(base64) {
    ...
    try {
      await axios.post('http://' + ip + ':' + port + '/barcodescanner', {
        data: form,
      }).then((response) => {
        setLoading(false);
        console.log(response.data);
        if (response.data["Error"]) {
          navigation.push("Scan Error", {
      ...
      // renderScannedItems([{'product_name': 'Error in scan', 'category': 'Error in scan', 'co2_total_per_kg': "Error in scan"}]);
      navigation.push("Scan Error");
      console.log(err);
    }
  }
